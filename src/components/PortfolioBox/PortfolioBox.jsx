...
function PortfolioBox({flipHandler, portfolioBoxSide, logo, title, portfolioName, creator, tippyContent, assetsImg, indexTokenBalance, currentBnbPrice, numberOfInvestors, indexVaultBalance, tokens, isWalletConnected, toggleConnectWalletModal, toggleCreateModal}) {
        ...
        <div className="portfolio-box">
            ...
            <div className='portfolio-box-front'>
                <div className="level1">
                    ...
                    <Tippy placement='top' animation='scale' animateFill={true} content={tippyContent}>
                        <img src={InfoImg} alt="" className='portfolio-box-front-info-img cursor-pointer' />
                    </Tippy>
                </div>

                <img className="portfolio-box-assets-img cursor-pointer" src={assetsImg} alt="" title='Click to see assets allocation' onClick={() => flipHandler(portfolioName)} />

                <div className="portfolio-box-user-amount">
                        <span>Amount</span>
                        <span style={formatDecimal(indexTokenBalance) > 0 ? {color: '#564dd0'} : {color: '#b3b3b3'}}> {formatDecimal(indexTokenBalance) == 0 ? '0' : formatDecimal(indexTokenBalance)} {portfolioName} </span>
                </div>

                <div className="portfolio-box-user-balance">
                    <span>Value</span>
                    <span style={formatDecimal(indexTokenBalance) > 0 ? {color: '#564dd0'} : {color: '#b3b3b3'}}> $ { (indexTokenBalance * currentBnbPrice).toLocaleString('en-US', {maximumFractionDigits: 1}) } </span>
                </div>
                ...
                <button className="btn fn-md" data-portfolio-name={portfolioName} onClick={isWalletConnected ? toggleCreateModal : toggleConnectWalletModal}>
                ...
                <div className="portfolio-data">
                    ...
                    <Tippy placement='top' animation='scale' arrow={false} content={'Amount Invested In Portfolio'} >
                        <div className="right">
                            <img src={DollarImg} alt="" />
                            <span className="marketcap fn-sm">{ (indexVaultBalance * currentBnbPrice).toLocaleString('en-US', {maximumFractionDigits:1}) }</span>
                        </div>
                    </Tippy>
                </div>
            ...
            <div className="portfolio-box-back">
                <img src={CrossImg} alt="" id="portfolio-box-back-cross" onClick={() => flipHandler(portfolioName)} />
                <h2>Allocation</h2>
